...
with st.form(key='submit_data'):
    ...
    weight = st.weight_input("Weight", 0, 200) # Input the weight
    submitted = st.form_submit_button("Submit")
if submitted : # if button clicked
        if date in data['date'].values : # if the date specified already exist in our data
            data.loc[data['date']==date,'weight'] = weight # then we replace the weight
        else :  # if the date specified does not exist in our data
            new_record = pd.DataFrame({'weight':[weight],'date':[date]}) # create a new record
            data= pd.concat([data,new_record],axis=0) # combine it with existing data
        
        data['date'] = pd.to_datetime(data['date']).dt.normalize() # dt.normalize to remove time component since we only need the date 
        data.to_csv('historical_weight.csv',index=False) # save changes back to the file
